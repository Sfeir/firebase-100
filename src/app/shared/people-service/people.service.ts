...
export class PeopleService {
  ...
  constructor(private _http: Http) {
    ...
    let baseUrl = `${environment.backend.protocol}://${
  ...
  fetchRandom(): Observable<any> {
    return this._http.get(this._backendURL.randomPeople).map(res => res.json());
  }

  fetchOne(id: string): Observable<any> {
    return this._http
      .get(this._backendURL.onePeople.replace(':id', id))
      .map(res => res.json());
  }
  ...
  update(person: any): Observable<any> {
    return this._http
      .put(this._backendURL.onePeople.replace(':id', person.id), person)
      .map((res: Response) => res.json());
  }

  create(person): Observable<any> {
    return this._http
      .post(this._backendURL.allPeople, JSON.stringify(person), this._options())
      .map((res: Response) => res.json());
  }
